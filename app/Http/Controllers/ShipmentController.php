...
{
  public function __construct(Shipment $shipment,EmailController $email, Item $item, Orders $order, 
...
public function makeDelivery($order_no,$shipment_id){
  ...
  return redirect()->intended('shipment/view-details/'.$order_no.'/'.$shipment_id);
    ...
    **/
    public function getShipmentByOrderNo(Request $request){
      ...
      //$order_no = 3;
      $location_id = "PL";
      $shipment = $this->shipment->getShipmentByOrderNo($order_no, $location_id);
      $state = false;
      //echo count($shipment);
      if(!empty($shipment)){
        $state = true;
      }
      return response()->json(['state'=>$state, 'shipments'=>$shipment]);
    }
    /**
    * @author: Nam Nguyen
    **/
    ...
    public function manualAllocate(Request $request){
     ...
     if($shipment_id==-1){
      ...
      foreach ($data as $key => $detail) {
        $unpacked_qty = $this->order->calculateAdditionalPackedQuantity($order_no,$detail[0],$location_id);
        $add_packed_qty = 0;
  ...
  //     $unpacked_qty = $this->order->calculateAdditionalPackedQuantity($order_no,$orderDetail[0],$location_id);

  //     if($unpacked_qty>=$orderDetail[1]){
  //       DB::table('shipment_details')->where('id',$orderDetail[2])->increment('packed_qty',$orderDetail[1]);
       
  //     }else{
  //       DB::table('shipment_details')->where('id',$orderDetail[2])->increment('packed_qty',$unpacked_qty);
