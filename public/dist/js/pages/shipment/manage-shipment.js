$(document).ready(function(){
 ...
 SHIPMENT.writeToTable = function(data){
 	...
 	$.each(data.shipments,function(index, shipment){
 		...
 		var remove_btn = '<button class="btn btn-danger deleteShipment" shipment-id="'+shipment.id+'">Delete Shipment</button>';
 		var edit_btn = '<button class="btn btn-info editShipment" shipment-id="'+shipment.id+'">Edit Shipment</button>';
 		 if(shipment.tracking_number==""){
 		 	$('<td>').html('<button class="btn btn-success btnEditTracking" shipment-id="'+shipment.id+'">Tracking and Shipment method</button>'+remove_btn+edit_btn).addClass("text-right").attr('colspan',4).appendTo(tr);
 		 }else{
 			$('<td>').html('<button class="btn btn-primary btnEditTracking" shipment-id="'+shipment.id+'" data-tracking-number="'+shipment.tracking_number+'" shipping-method="'+shipment.shipping_method+'">Tracking and Shipment method</button>'+remove_btn).addClass("text-right").attr('colspan',4).appendTo(tr);
 		}
 		...
 		$('<td>').html('<input type="number" name="packed_qty" value="'+detail.packed_qty+'" readonly>').appendTo(tr);
 		$('<td>').html(detail.shipped_qty).appendTo(tr);
...
SHIPMENT.add_new_shipment_control = function(data){
		var tr = $("<tr>").addClass('newShipmentRow');
 		...
 		$('<td>').html(detail.packed_qty).appendTo(tr);
 		$('<td>').html(detail.shipped_qty).appendTo(tr);
	 	...
	 	shipment_tbody.append('<tr class="saveBtnRow newShipmentRow" shipment-id="-1" hidden><td colspan="8" class="text-right"><button class="btn btn-primary saveManualAllocate" btn-shipment-id="-1">Save</button><button class="btn btn-danger btnHideNewShipmentControl">Cancel</button></td></tr>');
}
