...
<body>
  ...
  <div style="width:450px; float:right;">
    ...
    <div style="margin-top:70px; text-align:right;">
      ...
      <div style="font-size:16px;">{{$supplierData->supp_name}}</div>
      <div style="font-size:16px;">{{$supplierData->address}}</div>
      <div style="font-size:16px;">Contact : {{$supplierData->contact}}</div>
    </div>
    <br />
  ...
  @foreach ($invoiceData as $item)
    <div style="padding:0px 20px; margin-top:15px;">
      <div style="width:200px; font-size:14px; float:left;">{{$item->item_code}}</div>
      <div style="width:460px; float:left; font-size:14px; text-align:justify;">{{$item->description}}</div>
      <div style="width:200px; text-align:center; font-size:14px; float:left;">{{$item->quantity_ordered}}</div>

    </div>
   <br/>
<div style="clear:both"></div>
  <?php
    $sum = $item->quantity_ordered+$sum;
  ?>
  @endforeach  
    <div style="clear:both"></div>
    <br/>
